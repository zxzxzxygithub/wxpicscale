package com.mofanbaby.qiaotalk.widget;
...
public class ImageWatcher extends FrameLayout implements GestureDetector.OnGestureListener, ViewPager.OnPageChangeListener {
    ...
    private SimpleDraweeView iSource;
    private SimpleDraweeView iOrigin;

    private int mErrorImageRes = R.drawable.default_portrait_detail;
    ...
    private List<SimpleDraweeView> mImageGroupList;
     ...
     */
    public void show(SimpleDraweeView i, List<SimpleDraweeView> imageGroupList, final List<String> urlList) {
    ...
    class ImagePagerAdapter extends PagerAdapter {
        ...
        private final SparseArray<SimpleDraweeView> mImageSparseArray = new SparseArray<>();
        ...
        public Object instantiateItem(ViewGroup container, int position) {
            ...
            GenericDraweeHierarchy hierarchy = builder
                    .setPlaceholderImage(R.drawable.default_portrait_detail)
                    .setPlaceholderImageScaleType(ScalingUtils.ScaleType.CENTER_CROP)
                    .setActualImageScaleType(ScalingUtils.ScaleType.FIT_CENTER)
                    .build();
            photoDraweeView.setHierarchy(hierarchy);

            itemView.addView(photoDraweeView);
            mImageSparseArray.put(position, photoDraweeView);
            ...
            if (setDefaultDisplayConfigs(photoDraweeView, position, hasPlayBeginAnimation)) {
        ...
        private boolean setDefaultDisplayConfigs(final SimpleDraweeView simpleDraweeView, final int pos, boolean hasPlayBeginAnimation) {
            String url = mUrlList.get(pos);
            boolean isFindEnterImagePicture = false;

            ViewState.write(simpleDraweeView, ViewState.STATE_ORIGIN).alpha(0).scaleXBy(1.5f).scaleYBy(1.5f);
            if (pos < mImageGroupList.size()) {
                SimpleDraweeView originRef = mImageGroupList.get(pos);
                if (pos == initPosition && !hasPlayBeginAnimation) {
                    isFindEnterImagePicture = true;
                    iSource = simpleDraweeView;
                    iOrigin = originRef;
                }
                ...
                simpleDraweeView.setTranslationY(locationYOfFullScreen);
                simpleDraweeView.getLayoutParams().width = originRef.getWidth();
                simpleDraweeView.getLayoutParams().height = originRef.getHeight();

                ViewState.write(simpleDraweeView, ViewState.STATE_ORIGIN).width(originRef.getWidth()).height(originRef.getHeight());
                ...
                if (bmpMirror != null) {
                    ...
                    ViewState vsThumb = ViewState.write(simpleDraweeView, ViewState.STATE_THUMB).width(bmpMirrorWidth).height(bmpMirrorHeight)
                            .translationX((mWidth - bmpMirrorWidth) / 2).translationY((mHeight - bmpMirrorHeight) / 2);
                    simpleDraweeView.setImageDrawable(bmpMirror);

                    if (isFindEnterImagePicture) {
                        animSourceViewStateTransform(simpleDraweeView, vsThumb);
                    } else {
                        ViewState.restore(simpleDraweeView, vsThumb.mTag);
                    }
                }
            }
            ...
            ControllerListener controllerListener = new BaseControllerListener<ImageInfo>() {
                ...
                public void onFinalImageSet(
                    ...
                    int resourceImageWidth = imageInfo.getWidth();
                    int resourceImageHeight = imageInfo.getHeight();
                    ...
                    if (resourceImageWidth * 1f / resourceImageHeight > mWidth * 1f / mHeight) {
                        ...
                        simpleDraweeView.setTag(R.id.image_orientation, "horizontal");
                    } else {
                        ...
                        simpleDraweeView.setTag(R.id.image_orientation, "vertical");
                    }
                    notifyItemChangedState(pos, false, false);

                    ViewState vsDefault = ViewState.write(simpleDraweeView, ViewState.STATE_DEFAULT).width(sourceDefaultWidth).height(sourceDefaultHeight)
                            .translationX(sourceDefaultTranslateX).translationY(sourceDefaultTranslateY);
                    if (isPlayEnterAnimation) {
                        animSourceViewStateTransform(simpleDraweeView, vsDefault);
                    } else {
                        ViewState.restore(simpleDraweeView, vsDefault.mTag);
                        simpleDraweeView.setAlpha(0f);
                        simpleDraweeView.animate().alpha(1).start();
                    }
                }
